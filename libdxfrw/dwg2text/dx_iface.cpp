    ...
    if (success) {
        for (std::list<DRW_Entity*>::const_iterator it =
            ...
            case DRW::TEXT:
                ...
                break;
            default:
                std::cout << "eType: " << static_cast<int>(e->eType) << std::endl;
                break;
            }
        }

        std::cout << "Begin Dump All" << std::endl;
        for (std::list<DRW_Entity*>::const_iterator it2 = cData->mBlock->ent.begin(); it2 != cData->mBlock->ent.end(); ++it2) 
        {
            DRW_Entity *e = *it2;
            std::cout << "e, type: " << e->eType << " linetype: " << e->lineType << ", layer: " << e->layer << std::endl;
